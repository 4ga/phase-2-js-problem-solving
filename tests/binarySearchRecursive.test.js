import {describe, it, expect} from "vitest";
import {binarySearchRecursive} from "../src/binarySearchRecursive.js"

describe("Binary search recursive", () => {
 it("finds 7 in [1, 3, 5, 7, 9] and returns 3", () => {
  expect(binarySearchRecursive([1, 3, 5, 7, 9], 7)).toBe(3);
 });

 it("finds 1 in [1, 3, 5, 7, 9] and returns 0", () => {
  expect(binarySearchRecursive([1, 3, 5, 7, 9], 1)).toBe(0);
 });

 it("finds 9 in [1, 3, 5, 7, 9] and returns 4", () => {
  expect(binarySearchRecursive([1, 3, 5, 7, 9], 9)).toBe(4);
 });

 it("finds 4 in [1, 3, 5, 7, 9] and returns -1", () => {
  expect(binarySearchRecursive([1, 3, 5, 7, 9], 4)).toBe(-1);
 });

 it("return -1 for []", () => {
  expect(binarySearchRecursive([], 1)).toBe(-1);
 });
 
});
